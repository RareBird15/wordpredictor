...
_OrtCreateTensorWithDataAsOrtValueFunc = ctypes.CFUNCTYPE(
	c_void_p, c_void_p, c_void_p, c_size_t, POINTER(c_longlong), c_size_t, c_int, POINTER(c_void_p)
)
...
def _load_ort_dlls(dll_dir: str):
	...
	vtable = ctypes.cast(api_base.contents, POINTER(POINTER(c_void_p))).contents
...
class OnnxLstmPredictor:
	...
	def _init_ctypes_session(self, model_path: str):
		...
		set_opt = _get_func(
			self._api_ptr, _IDX_SetSessionGraphOptimizationLevel, _OrtSetSessionGraphOptimizationLevelFunc
		)
	...
	def _predict_ctypes(self, input_ids: list[int], top_k: int) -> list[tuple[str, float]]:
		...
		create_tensor = _get_func(
			self._api_ptr, _IDX_CreateTensorWithDataAsOrtValue, _OrtCreateTensorWithDataAsOrtValueFunc
		)
		...
		# Create run options
		run_opts = c_void_p()
		create_run_opts = _get_func(self._api_ptr, _IDX_CreateRunOptions, _OrtCreateRunOptionsFunc)
		...
		# Get output data
		get_data = _get_func(self._api_ptr, _IDX_GetTensorMutableData, _OrtGetTensorMutableDataFunc)
		data_ptr = c_void_p()
